...
function filePathFromDatabaseUrl(raw: string): string {
	return (raw || 'file:/data/config/sphere86.db').replace(/^file:/, '');
}

const dbFilePath = filePathFromDatabaseUrl(env.DATABASE_URL || 'file:/data/config/sphere86.db');
...
export const BOX86_CONFIG_BASE_PATH = env.BOX86_CONFIG_BASE_PATH || SPHERE86_DATA_ROOT;
